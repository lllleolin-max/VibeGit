...
export interface GitSshTransport {
  // This value is generated by the trusted main-process provider. It is never
  // accepted from the renderer or inherited from the user's environment.
  sshCommand: string
}
...
export class GitEngine {
  ...
  async pushCheckpoint(
    ...
    if (transport && (!remoteUrl.startsWith('ssh://') || transport.sshCommand.length === 0 || transport.sshCommand.length > 10_000 || /[\r\n]/.test(transport.sshCommand))) {
      throw new VibeGitError('INVALID_SSH_TRANSPORT', 'SSH 备份连接配置无效')
    }
    const remoteEnvironment = transport ? { GIT_SSH_COMMAND: transport.sshCommand } : undefined
    ...
    const advertised = await this.runner.run(projectPath, ['ls-remote', '--heads', remoteUrl, remoteRef], {
      ...(remoteEnvironment ? { env: remoteEnvironment } : {}),
      timeoutMs: 120_000,
      maxOutputBytes: 4 * 1024 * 1024
    })
